...
class tmi:
    ...
    def write_part(self, table, data):
        ...
        try:
            ...
            # Executing a SQL query to insert datetime into csv file
            with open('csv/%s.csv' % (table), mode='wb') as w_csv:
                wr_csv = csv.writer(w_csv, delimiter=';', quoting=csv.QUOTE_MINIMAL)
                for line in data:
                    ...
                    wr_csv.writerow(line)
            
            # Slower executing a SQL query insert into database
            # with open('csv/%s.csv' % (table), mode='rb') as r_csv:
            #     cursor.copy_from(r_csv, table, sep=';', null='', columns=helper.column_form(table))
            #     connection.commit()

            # Faster executing a SQL query insert into database
            import_query = """COPY %s (%s) FROM '%s/csv/%s.csv' WITH (FORMAT csv, NULL '', DELIMITER ';', FORCE_NULL(%s));""" % (table, helper.column(table), os.path.abspath(os.getcwd()), table, helper.column(table))
            if import_query:
            ...
            if (connection):
                cursor.close()
                connection.close()
                print "COPY %d RECORDS INTO %s FROM '%s/csv/%s.csv' DELIMITERS ';', '\\n' NULL AS '';" % (len(data), table, os.getcwd(), table)
